...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if(tracking_flag == -1){
            ...
            // ******************** START YOUR CODE HERE ******************** //
            ok = myTracker.update(mGray, myROI);
            if (!ok) {
                Imgproc.putText(mRgba, "Tracking failure occurred!", new Point(10, 30), Core.FONT_HERSHEY_PLAIN, 0.75, new Scalar(0, 255, 0), 2);
            }

            double fps = Core.getTickFrequency() / (Core.getTickCount() - start);
            Imgproc.putText(mRgba, "FPS@" + String.format("%.2f", fps), new Point(400, 30), Core.FONT_HERSHEY_PLAIN, 0.75, new Scalar(0, 255, 0), 2);
